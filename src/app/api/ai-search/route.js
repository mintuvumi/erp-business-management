...
export async function GET(req) {
  try {
    ...
    const bankBalance = banks.reduce(
      (sum, b) =>
        sum + Number(b.currentBalance || 0),
      0
    );

    const totalProfit = sales.reduce(
      (sum, s) =>
        sum + Number(s.totalProfit || 0),
      0
    );
    ...
    if (q.includes("profit") || q.includes("লাভ")) {
      ...
      answer = `Total profit ৳ ${money(
        netProfit
      )}। এই মাসের profit ৳ ${money(
        monthlyProfit
      )}।`;
      ...
      answer = `Total bank balance ৳ ${money(
        bankBalance
      )}। মোট bank account ${
        banks.length
      } টি।`;
      ...
      answer = `Total employee ${
        employees.length
      } জন। Present ${
        employees.filter((e) => e.presentToday)
          .length
      } জন, absent ${
        employees.filter((e) => !e.presentToday)
          .length
      ...
      suggestions = [
        "Salary sheet এবং attendance নিয়মিত update করুন।",
      ];
    } else {
