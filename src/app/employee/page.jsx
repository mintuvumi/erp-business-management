    ...
    <div className="space-y-5">
      <div className="bg-white border rounded-[28px] p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-5">
          ...
          <select
            value={String(form.presentToday)}
            onChange={(e) =>
              setForm({ ...form, presentToday: e.target.value === "true" })
            }
            className="border rounded-xl p-3"
          >
          ...
          <input
            type="number"
            value={form.monthlyLeave}
            onChange={(e) =>
              setForm({ ...form, monthlyLeave: Number(e.target.value) || 0 })
            }
            placeholder="Monthly Leave"
            className="border rounded-xl p-3"
          />

          <input
            type="number"
            value={form.yearlyLeave}
            onChange={(e) =>
              setForm({ ...form, yearlyLeave: Number(e.target.value) || 0 })
            }
          ...
          <select
            ...
            onChange={(e) =>
              setForm({ ...form, workProgress: e.target.value })
            }
            className="border rounded-xl p-3"
          >
          ...
          <textarea
            ...
            placeholder="Note"
            className="md:col-span-4 border rounded-xl p-3 min-h-[80px]"
          />
        </div>

        <button
          onClick={saveEmployee}
          className="mt-4 inline-flex items-center gap-2 bg-blue-500 text-white px-5 py-3 rounded-xl hover:bg-blue-600"
        >
          <Plus size={16} />
          Save Employee
        </button>
          ...
          <table className="w-full min-w-[950px] text-sm">
            ...
            <tbody>
              {employees.length === 0 ? (
                ...
                employees.map((emp) => (
                  <tr key={emp._id} className="border-t hover:bg-blue-50/40">
                    ...
                    <td className="p-4 text-center">
                      {emp.presentToday ? "Present" : "Absent"}
                    </td>
